...
# Progress goes to the information stream rather than Write-Host, so it can be redirected,
# captured or suppressed by a caller. $InformationPreference keeps it visible by default.
...
function Write-Status {
	...
	if ($Colour -and -not $env:NO_COLOR) {
		Write-Information "$($script:Ansi[$Colour])$Message$($script:Ansi.Reset)"
	} else {
